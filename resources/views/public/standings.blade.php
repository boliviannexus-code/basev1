...
<main class="public-shell content-section">
    ...
    <table class="public-table"><thead><tr><th>Pos</th><th>Equipo</th><th>PJ</th><th>GF</th><th>GC</th><th>DG</th><th>Pts</th></tr></thead><tbody>@foreach($standings as $row)<tr><td>{{ $row['position'] }}</td><td>{{ $row['team_name'] }}</td><td>{{ $row['played'] }}</td><td>{{ $row['goals_for'] }}</td><td>{{ $row['goals_against'] }}</td><td>{{ $row['goal_difference'] }}</td><td><strong>{{ $row['final_points'] }}</strong></td></tr>@endforeach</tbody></table>
</main>
