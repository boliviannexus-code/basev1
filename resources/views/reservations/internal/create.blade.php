...
@section('title', 'Nueva reserva interna | '.config('app.name', 'Base Admin'))
@section('page-title', 'Nueva reserva interna')
...
@section('content')
    ...
    @unless ($currentExchangeRate)
        <div class="alert alert-warning">
            Configura un tipo de cambio USD a BOB para esta empresa antes de registrar reservas.
        </div>
    @endunless

    <form
        method="POST"
        action="{{ route('internal-reservations.store') }}"
        data-check-in-form
        data-resources='@json($resources)'
        data-available-resources-url="{{ route('internal-reservations.available-resources') }}"
                    ...
                    <div class="card-body">
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <input class="form-control @error('main_guest.first_name') is-invalid @enderror @error('first_name') is-invalid @enderror" id="first_name" name="first_name" value="{{ old('main_guest.first_name', old('first_name')) }}" autocomplete="new-password" data-browser-autofill-off required data-main-guest-name>
                                <div class="invalid-feedback">{{ $errors->first('main_guest.first_name') ?: $errors->first('first_name') }}</div>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label" for="last_name">Apellido</label>
                                <input class="form-control @error('main_guest.last_name') is-invalid @enderror @error('last_name') is-invalid @enderror" id="last_name" name="last_name" value="{{ old('main_guest.last_name', old('last_name')) }}" autocomplete="new-password" data-browser-autofill-off required data-main-guest-last-name>
                                <div class="invalid-feedback">{{ $errors->first('main_guest.last_name') ?: $errors->first('last_name') }}</div>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label" for="phone">Telefono</label>
                                <input class="form-control @error('main_guest.phone') is-invalid @enderror @error('phone') is-invalid @enderror" id="phone" name="phone" value="{{ old('main_guest.phone', old('phone')) }}" autocomplete="off">
                                <div class="invalid-feedback">{{ $errors->first('main_guest.phone') ?: $errors->first('phone') }}</div>
                            </div>
                        </div>
                    </div>
                </x-ui.card>
                ...
                <x-ui.card class="mt-3" title="Habitaciones reservadas">
                    <x-slot:actions>
                        <button class="btn btn-outline-primary btn-sm" type="button" data-check-in-add-stay>
                            <i class="ti ti-plus me-1"></i>Agregar recurso
                        </button>
                    </x-slot:actions>
                ...
                <x-ui.card title="Reserva">
                    <div class="card-body">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input class="form-control @error('check_in_date') is-invalid @enderror" id="check_in_date" name="check_in_date" type="date" min="{{ today()->toDateString() }}" value="{{ old('check_in_date', $initial['check_in_date']) }}" autocomplete="off" required data-check-in-date>
                    ...
                    <x-slot:footer>
                        <div class="d-flex justify-content-end gap-2">
                            ...
                            <button class="btn btn-success" type="submit" @disabled(! $currentExchangeRate)>
                                <i class="ti ti-calendar-plus me-1"></i>Registrar reserva
                            </button>
                        </div>
                    </x-slot:footer>
                </x-ui.card>
