...
@section('content')
    <div data-refresh-container>
        ...
        <x-ui.table-card title="Inscripciones por torneo">
            ...
            @else
                <ul class="nav nav-tabs registration-filter-tabs registration-division-tabs mb-3" role="tablist" aria-label="Divisiones">
                    @foreach ($divisions as $division)
                        @php
                            $divisionTabId = 'registration-division-tab-'.$division->id;
                            $divisionCount = $registrationsByTournament
                                ->flatten(1)
                        ...
                        <li class="nav-item" role="presentation">
                            <button class="nav-link registration-filter-tab @if ($isActiveDivision) active @endif" id="{{ $divisionTabId }}-tab" data-bs-toggle="tab" data-bs-target="#{{ $divisionTabId }}" type="button" role="tab" aria-controls="{{ $divisionTabId }}" aria-selected="{{ $isActiveDivision ? 'true' : 'false' }}">
                                <span class="registration-tab-title">{{ $division->name }}</span>
                                <span class="badge registration-tab-badge">{{ $divisionCount }}</span>
                            </button>
                        </li>
                    @endforeach
                </ul>
                ...
                <div class="tab-content registration-division-content">
                    @foreach ($divisions as $division)
                        @php
                            $divisionTabId = 'registration-division-tab-'.$division->id;
                            $divisionTournaments = $tournamentsByDivision->get($division->id, collect())
                                ->filter(fn ($tournament) => $registrationsByTournament->has($tournament->id))
                                ->values();
                            $isActiveDivision = (int) $activeDivisionId === $division->id;
                        @endphp
                        <div class="tab-pane fade @if ($isActiveDivision) show active @endif" id="{{ $divisionTabId }}" role="tabpanel" aria-labelledby="{{ $divisionTabId }}-tab" tabindex="0">
                            @if ($divisionTournaments->isEmpty())
                                <div class="text-body-secondary py-4">No hay torneos con inscripciones en esta division.</div>
                            @else
                                <ul class="nav nav-tabs registration-filter-tabs registration-tournament-tabs compact mb-3" role="tablist" aria-label="Torneos de {{ $division->name }}">
                                    @foreach ($divisionTournaments as $tournament)
                                        ...
                                        @endphp
                                        <li class="nav-item" role="presentation">
                                            <button class="nav-link registration-filter-tab compact @if ($loop->first) active @endif" id="{{ $tabId }}-tab" data-bs-toggle="tab" data-bs-target="#{{ $tabId }}" type="button" role="tab" aria-controls="{{ $tabId }}" aria-selected="{{ $loop->first ? 'true' : 'false' }}">
                                ...
                                <div class="tab-content">
                                    @foreach ($divisionTournaments as $tournament)
                                        @php
                                            $tabId = 'tournament-registration-tab-'.$tournament->id;
                                            $registrations = $registrationsByTournament->get($tournament->id, collect());
                                            $registrationsByCategory = $registrations
                                                ->groupBy(fn ($registration) => $registration->category_id ?: 'none');
                                        @endphp
